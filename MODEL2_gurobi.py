...
def m2_run_cutting_stock_optimizer(run_id):
    # Generate new data
# =============================================================================
...
# =============================================================================
    ...
    for j in range(n):
        model.addConstr(W * YLR[j] <= l[j] * z[j] - quicksum(X[i, j] * r[i] for i in range(m)))
    
    # 4 Ensures proper classification of waste and retail
    for j in range(n):
        model.addConstr(WL[j] + YLR[j] * BIGM >= l[j] * z[j] - quicksum(X[i, j] * r[i] for i in range(m)))
    ...
    if model.status == GRB.OPTIMAL:
        ...
        print("      CUTTING PLAN:")
        for j in range(n):
            print(f"      Bar {j+1} (Length: {l[j]}, Waste: {abs(WL[j].X):.0f})")    # Does not calculate Leftover length (LOl[j])
            for i in range(m):
                if X[i, j].X > 0.5:
...
for run in range(1, num_runs + 1):
    m2_run_cutting_stock_optimizer(run)
