...
# Parameters
minB, maxB = 15, 40    # Range for number of bars
minO, maxO = 3, 10      # Range for number of orders
MaxLength = 300         # Maximum bar length
...
#MODEL3
CR = 200                # Cost of returning a retail to stock
...
def m3_run_cutting_stock_optimizer(run_id):
    # Generate new data
# =============================================================================
...
# =============================================================================
    ...
    model.setObjective(quicksum(WL[j] * CW + YLR[j] * CR for j in range(n)), GRB.MINIMIZE)
    ...
    for j in range(n):
        model.addConstr(W - LOl[j] + W * (YLR[j] - 1) <= 0)
    ...
    if model.status == GRB.OPTIMAL:
        ...
        print("      CUTTING PLAN:")
        for j in range(n):
            print(f"      Bar {j+1} (Length: {l[j]}, Leftover: {LOl[j].X:.0f}, Waste: {abs(WL[j].X):.0f})")     # Leftover  LUl[j is not calculated accurately, if a bar wasn't used
            for i in range(m):
                if X[i, j].X > 0.5:
...
for run in range(1, num_runs + 1):
    m3_run_cutting_stock_optimizer(run)
